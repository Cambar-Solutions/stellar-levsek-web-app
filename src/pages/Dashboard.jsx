          ...
          <div className="md:hidden p-3 sm:p-4 space-y-3 sm:space-y-4">
            {filteredDebtors.length === 0 ? (
              ...
              filteredDebtors.map((debtor) => (
                <div
                  key={debtor.id}
                  className="bg-gray-50 dark:bg-gray-700 rounded-lg sm:rounded-xl p-3 sm:p-4 space-y-2 sm:space-y-3"
                >
                  {/* Header */}
                  <div className="flex items-center gap-2 sm:gap-3 pb-2 sm:pb-3 border-b border-gray-200 dark:border-gray-600">
                    <Avatar name={debtor.name} className="flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-sm sm:text-base text-gray-900 dark:text-white truncate">
                        {debtor.name}
                      </p>
                      <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 truncate">{debtor.email}</p>
                    </div>
                    <Badge
                      variant={
                        debtor.status === 'verified'
      ...
      <CardContent className="p-4 sm:p-6">
        <div className="flex items-start justify-between">
          <div className="min-w-0 flex-1">
            <p className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-300 mb-1">{title}</p>
            <p className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white break-words">{value}</p>
          </div>
          <div
            className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-gradient-to-br ${colorClasses[color]} flex items-center justify-center text-white shadow-lg flex-shrink-0`}
          >
            {icon}
          </div>
        </div>
